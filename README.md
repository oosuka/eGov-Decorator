...
3. 拡張アイコンのバッジで状態表示
4. 対象ページで拡張アイコンクリック時に色・ハイライトレベル設定ポップアップを表示
...
2. ハイライトレベル、背景色、文字色を選択して「保存」
3. 「デフォルトに戻す」でレベルと色を既定値に戻す
...
- 保存項目:
  ...
  - `highlightBgColor`
  - `highlightTextColor`
- `storage` の読み出し結果が欠損・不正でも、既定値または legacy キーから安全にフォールバック
...
7. ポップアップでハイライトレベルと色を変更し、即時反映されることを確認
...
- 安全コンテナ（`p` / `div` / `li` など）が明示的に見つからない場合は、ノードまたぎ処理を行いません。
- ノードまたぎ処理を含め、ハイライト対象は対応する `（...）` ペアのみです（未閉じ `（` と未対応 `）` はハイライトしません）。
